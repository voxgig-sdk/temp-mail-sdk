...
//     (\`new TempMailSDK(...)\` / \`TempMailSDK.test(...)\` become
//     \`TempMailSDK.test({ entity: ... })\`), transpiled to JS, and run
...
const TEST_SEED = {"entity":{"email":{"test01":{"id":"test01"}},"mailbox":{"test01":{"id":"test01"}}}}
...
// rewrite real client construction (\`new TempMailSDK(...)\` and any
// \`TempMailSDK.test(...)\`) to a seeded test-mode client, and give
...
function rewriteForRun(code: string): string {
  ...
  out = out.replace(/new\s+TempMailSDK\s*\([^)]*\)/g, () => SEEDED_CTOR)
  out = out.replace(/TempMailSDK\.test\s*\([^)]*\)/g, () => SEEDED_CTOR)
