...
// TempMail SDK struct utility test
//
...
class StructUtilityTest extends TestCase
{
    ...
    {
        if (self::$run === null) {
            $runner = TempMailOmni::makeStructRunner(
        ...
        $this->runsection(
            ...
            function ($input) {
                ...
                if (is_object($store) && $haspath && Struct::undef() !== $val) {
                    ...
                    if (json_encode(TempMailOmni::structfix($now, Struct::undef()))
                        !== json_encode(TempMailOmni::structfix($val, Struct::undef()))) {
