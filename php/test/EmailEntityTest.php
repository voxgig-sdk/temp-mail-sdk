...
class EmailEntityTest extends TestCase
{
    ...
    public function test_stream(): void
    {
        ...
        $base = TempMailSDK::test($seed, null);
        $seen = iterator_to_array($base->Email(null)->stream("list", null, null), false);
        $this->assertCount(3, $seen);

        // Inbound: streaming active -> yields each item from the feature.
        $cfg = TempMailConfig::shared_config();
        if (isset($cfg["feature"]) && is_array($cfg["feature"]) && isset($cfg["feature"]["streaming"])) {
            $sdk = TempMailSDK::test($seed, ["feature" => ["streaming" => ["active" => true]]]);
            $got = [];
    ...
    public function test_basic_flow(): void
    {
        ...
        if (!empty($setup["synthetic_only"])) {
            $this->markTestSkipped("live entity test uses synthetic IDs from fixture — set TEMP_MAIL_TEST_EMAIL_ENTID JSON to run live");
            return;
        }
        $client = $setup["client"];
...
function email_basic_setup($extra)
{
    ...
    $entid_env_raw = getenv("TEMP_MAIL_TEST_EMAIL_ENTID");
