...
{
    private static function utility(): TempMailUtility
    {
        return new TempMailUtility();
    }
    ...
    private static function ctx(array $over = []): TempMailContext
    {
        $utility = $over['utility'] ?? self::utility();
        $client = $over['client'] ?? new PlClient(['base' => 'http://h']);
        $ctx = new TempMailContext([
            'client' => $client,
            'utility' => $utility,
        ], null);
        $ctx->op = new TempMailOperation(['name' => 'load', 'entity' => 'x']);
        foreach ($over as $k => $v) {
    ...
    {
        return ($err instanceof TempMailError) ? $err->sdk_code : '';
    }
    ...
    public function test_make_point_rejects_a_disallowed_operation(): void
    {
        ...
        $ctx->op = new TempMailOperation(['name' => 'nope', 'entity' => 'x']);
        [$point, $err] = TempMailMakePoint::call($ctx);
    ...
    public function test_make_point_rejects_an_operation_with_no_endpoints(): void
    {
        ...
        $ctx->op = new TempMailOperation(['name' => 'load', 'entity' => 'x', 'points' => []]);
        [$point, $err] = TempMailMakePoint::call($ctx);
        $this->assertNull($point);
    ...
    public function test_make_point_returns_the_single_point(): void
    {
        ...
        $ctx = self::ctx(['options' => ['allow' => ['op' => 'load,list,create,update,remove']]]);
        $ctx->op = new TempMailOperation(['name' => 'load', 'entity' => 'x', 'points' => [$point]]);
        [$got, $err] = TempMailMakePoint::call($ctx);
        $this->assertNull($err);
    ...
    {
        $preset = new TempMailSpec(['method' => 'GET']);
        $ctx = self::ctx();
        $ctx->out['spec'] = $preset;
        [$got, $err] = TempMailMakeSpec::call($ctx);
        $this->assertNull($err);
        $this->assertSame($preset, $got);
    }
    ...
    public function test_make_response_guards_missing_spec_response_result(): void
    {
        $ctx = self::ctx([
            'spec' => null,
            'response' => new TempMailResponse([]),
            'result' => new TempMailResult([]),
        ]);
        [, $err] = TempMailMakeResponse::call($ctx);
        $this->assertSame('response_no_spec', self::code($err));

        $ctx = self::ctx([
            'spec' => new TempMailSpec([]),
            'response' => null,
            'result' => new TempMailResult([]),
        ]);
        [, $err] = TempMailMakeResponse::call($ctx);
    ...
    {
        $ctx = self::ctx([
            'spec' => new TempMailSpec(['step' => 's']),
            'response' => new TempMailResponse(self::resp(404, null, ['x-a' => '1'])),
            'result' => new TempMailResult([]),
        ]);
        [, $err] = TempMailMakeResponse::call($ctx);
        $this->assertNull($err);
    ...
    public function test_make_response_2xx_parses_the_body_and_marks_ok(): void
    {
        $ctx = self::ctx([
            'spec' => new TempMailSpec(['step' => 's']),
            'response' => new TempMailResponse(self::resp(200, ['v' => 1])),
            'result' => new TempMailResult([]),
        ]);
        [, $err] = TempMailMakeResponse::call($ctx);
        $this->assertNull($err);
    ...
    public function test_make_response_records_to_ctrl_explain_when_explain_is_on(): void
    {
        $ctx = self::ctx([
            'spec' => new TempMailSpec(['step' => 's']),
            'response' => new TempMailResponse(self::resp(200, ['v' => 2])),
            'result' => new TempMailResult([]),
        ]);
        $ctx->ctrl->explain = ['on' => true];
        TempMailMakeResponse::call($ctx);
    ...
    {
        $preset = new TempMailResponse(self::resp(299));
        $ctx = self::ctx([
            'spec' => new TempMailSpec([]),
            'response' => new TempMailResponse([]),
            'result' => new TempMailResult([]),
        ...
        [$got, $err] = TempMailMakeResponse::call($ctx);
        $this->assertNull($err);
        $this->assertSame($preset, $got);
    }
    ...
    {
        $ctx = self::ctx(['spec' => null, 'result' => new TempMailResult([])]);
        [, $err] = TempMailMakeResult::call($ctx);
        $this->assertSame('result_no_spec', self::code($err));

        $ctx = self::ctx(['spec' => new TempMailSpec([]), 'result' => null]);
        [, $err] = TempMailMakeResult::call($ctx);
    ...
    public function test_make_result_list_op_wraps_resdata_into_entity_instances(): void
    {
        ...
        $ctx = self::ctx([
            'entity' => $entity,
            'spec' => new TempMailSpec(['step' => 's']),
            'result' => new TempMailResult(['ok' => true, 'resdata' => [['a' => 1], ['a' => 2]]]),
        ]);
        $ctx->op = new TempMailOperation(['name' => 'list', 'entity' => 'x']);
        [$result, $err] = TempMailMakeResult::call($ctx);
        $this->assertNull($err);
    ...
    public function test_make_result_empty_list_yields_empty_resdata(): void
    {
        ...
        $ctx = self::ctx([
            'entity' => $entity,
            'spec' => new TempMailSpec(['step' => 's']),
            'result' => new TempMailResult(['ok' => true, 'resdata' => []]),
        ]);
        $ctx->op = new TempMailOperation(['name' => 'list', 'entity' => 'x']);
        [$result, $err] = TempMailMakeResult::call($ctx);
        $this->assertNull($err);
    ...
    {
        $preset = new TempMailResult(['ok' => true]);
        $ctx = self::ctx([
            'spec' => new TempMailSpec([]),
            'result' => new TempMailResult([]),
        ]);
        $ctx->out['result'] = $preset;
        [$got, $err] = TempMailMakeResult::call($ctx);
        $this->assertNull($err);
        $this->assertSame($preset, $got);
    }
    ...
    public function test_make_request_guards_a_missing_spec(): void
    {
        ...
        [, $err] = TempMailMakeRequest::call($ctx);
    ...
    {
        $utility = self::utility();
        $boom = new TempMailError('boom', 'boom');
        $utility->fetcher = function (TempMailContext $_c, string $_u, array $_f) use ($boom): array {
            return [null, $boom];
        };
        $ctx = self::ctx([
            'utility' => $utility,
            'spec' => new TempMailSpec(['step' => 's', 'method' => 'GET', 'base' => 'http://h', 'parts' => ['a']]),
        ]);
        [$response, $err] = TempMailMakeRequest::call($ctx);
        $this->assertNull($err);
    ...
    {
        $utility = self::utility();
        $utility->fetcher = function (TempMailContext $_c, string $_u, array $_f): array {
            return [null, null];
        };
        $ctx = self::ctx([
            'utility' => $utility,
    ...
    public function test_make_request_a_normal_transport_response_is_wrapped(): void
    {
        ...
        [$response, $err] = TempMailMakeRequest::call($ctx);
        $this->assertNull($err);
        $this->assertInstanceOf(TempMailResponse::class, $response);
    ...
    {
        $utility = self::utility();
        $utility->make_fetch_def = function (TempMailContext $c): array {
            return [null, $c->make_error('fetchdef_boom', 'boom')];
        };
        $ctx = self::ctx([
            'utility' => $utility,
            'spec' => new TempMailSpec(['step' => 's', 'method' => 'GET']),
        ]);
        [$response, $err] = TempMailMakeRequest::call($ctx);
        $this->assertNull($err);
        $this->assertNotNull($response->err);
    ...
    {
        $preset = new TempMailResponse(self::resp(201));
        $ctx = self::ctx(['spec' => new TempMailSpec([])]);
        $ctx->out['request'] = $preset;
        [$got, $err] = TempMailMakeRequest::call($ctx);
        $this->assertNull($err);
        $this->assertSame($preset, $got);
    }
    ...
    {
        $ctx = self::ctx(['spec' => null]);
        [, $err] = TempMailMakeFetchDef::call($ctx);
    ...
    public function test_make_fetch_def_serialises_body_and_inits_missing_result(): void
    {
        ...
        [$fetchdef, $err] = TempMailMakeFetchDef::call($ctx);
        $this->assertNull($err);
    ...
    {
        $ctx = self::ctx(['result' => new TempMailResult(['ok' => true, 'resdata' => 42])]);
        $this->assertSame(42, TempMailDone::call($ctx));
    }

    public function test_done_raises_the_error_when_not_ok(): void
    {
        $ctx = self::ctx(['result' => new TempMailResult(['ok' => false])]);
        $this->expectException(TempMailError::class);
        TempMailDone::call($ctx);
    }

    public function test_make_error_returns_resdata_when_ctrl_throw_is_false(): void
    {
        $ctx = self::ctx(['result' => new TempMailResult(['ok' => false, 'resdata' => 'fallback'])]);
        $ctx->ctrl->throw_err = false;
        $this->assertSame('fallback', TempMailMakeError::call($ctx, null));
    }
    ...
    public function test_make_error_records_to_ctrl_explain(): void
    {
        ...
        $ctx->ctrl->throw_err = false;
        $ctx->ctrl->explain = ['on' => true];
        TempMailMakeError::call($ctx, null);
    ...
    public function test_feature_add_appends_in_call_order(): void
    {
        ...
        $a = new TempMailBaseFeature();
        $b = new TempMailBaseFeature();
        TempMailFeatureAdd::call($ctx, $a);
        TempMailFeatureAdd::call($ctx, $b);
        $this->assertSame([$a, $b], $client->features);
    }

    private static function named_feature(string $name): TempMailBaseFeature
    {
        $f = new TempMailBaseFeature();
        $f->name = $name;
        return $f;
    }
    ...
    public function test_feature_add_ordering_before_after_replace(): void
    {
        ...
        TempMailFeatureAdd::call($ctx, $miss);
    ...
    private static function auth_ctx(array $options, ?array $headers): TempMailContext
    {
        $client = new PlClient($options);
        return self::ctx([
            'client' => $client,
            'spec' => $headers === null ? null : new TempMailSpec(['headers' => $headers]),
        ]);
    }
    ...
    public function test_prepare_auth_guards_a_missing_spec(): void
    {
        ...
        [, $err] = TempMailPrepareAuth::call($ctx);
    ...
    {
        $ctx = self::ctx([
            'response' => new TempMailResponse(['headers' => null]),
            'result' => new TempMailResult([]),
        ]);
        TempMailResultHeaders::call($ctx);
