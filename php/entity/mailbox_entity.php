...
class MailboxEntity
{
    ...
    {
        if ($args) {
            $this->_data = TempMailHelpers::to_map(Struct::clone($args)) ?? [];
    ...
    {
        if ($args) {
            $this->_match = TempMailHelpers::to_map(Struct::clone($args)) ?? [];
     ...
     *
     * @param MailboxCreateData|array|null $reqdata Body data as an assoc-array;
     *   a typed MailboxCreateData names the shape.
     * @param mixed $ctrl Optional per-call control overrides.
     * @return Mailbox|array The created Mailbox as an assoc-array at the
     *   SDK boundary; throws TempMailError on failure (item-5 convention).
     */
        ...
        return $this->_run_op($ctx, function () use ($ctx) {
            if ($ctx->result) {
                if ($ctx->result->resdata) {
                    $this->_data = TempMailHelpers::to_map(Struct::clone($ctx->result->resdata)) ?? [];
                }
            }
        });
    }
