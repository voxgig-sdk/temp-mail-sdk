...
class TempMailNetsimFeature extends TempMailBaseFeature
{
    ...
    private function _simulate(TempMailContext $ctx, string $url, array $fetchdef, callable $inner): array
    {
        $opts = $this->options ?? [];
        $this->calls++;
        $call = $this->calls;
    ...
    private function _track(TempMailContext $ctx, array $applied): void
    {
        $client = $this->client;
