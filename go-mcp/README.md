...
[MCP](https://modelcontextprotocol.io) server exposing the TempMail SDK as
two agent tools — `temp-mail_list` and `temp-mail_load` — built on the
...
```sh
# 1. Build a native binary (-> dist/<os>-<arch>/temp-mail-mcp)
make build

# 2. Provide credentials via the environment
export TEMP_MAIL_APIKEY=sk_live_xxx

# 3a. Install into Claude Code over stdio (most common)
claude mcp add --scope user temp-mail \
  -- /absolute/path/to/temp-mail-mcp -transport stdio

# 3b. …or run a shared HTTP server instead
./temp-mail-mcp -transport http -addr :8080
```

Tool-call arguments (what an agent sends):

```jsonc
// temp-mail_list: first page of records
{ "entity": "email" }
{ "entity": "email", "query": { } }

// temp-mail_load: one record by id
...
1. **Build** the server from this `go-mcp/` directory:
   ...
   ```sh
   make build          # -> dist/<os>-<arch>/temp-mail-mcp
   ```
...
3. **Install it into Claude Code** (stdio transport):
   ...
   ```sh
   claude mcp add --scope user temp-mail \
     -- "$PWD"/dist/*/temp-mail-mcp -transport stdio
   ```

4. **Restart Claude Code.** The `temp-mail_list` and `temp-mail_load` tools now appear
   in new sessions. Ask the agent to *"list email using temp-mail"*
   and it calls `temp-mail_list` with `{"entity":"email"}`.
...
```sh
export TEMP_MAIL_APIKEY=sk_live_xxx            # API key
export TEMP_MAIL_BASE=https://api.example.com  # optional: override the API base URL
```
...
| Tool | Args | Returns |
|------|------|---------|
| `temp-mail_list` | `entity` (required), `query` (optional map) | First page of records as JSON |
| `temp-mail_load` | `entity` (required), `query` = `{id:N}` | Single record as JSON |
...
| Variable | Purpose |
|----------|---------|
| `TEMP_MAIL_APIKEY` | API key sent with every request. |
| `TEMP_MAIL_BASE` | Optional override of the API base URL. |
...
```sh
./temp-mail-mcp -transport http -addr :18080 &
...
curl -sN -X POST http://localhost:18080 \
  ...
  -d '{"jsonrpc":"2.0","id":2,"method":"tools/call","params":{"name":"temp-mail_load","arguments":{"entity":"email","query":{"id":1}}}}'
```
