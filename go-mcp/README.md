...
MCP server exposing the TempMail SDK as tools, built on the
...
| Tool | Args | Returns |
|------|------|---------|
| `temp-mail_list` | `entity`, optional `query` map | First page of records as JSON |
| `temp-mail_load` | `entity`, `query` (e.g. `{id:N}`) | Single record as JSON |
...
After install, restart Claude Code; the `temp-mail_list` and
`temp-mail_load` tools become available in new sessions.
