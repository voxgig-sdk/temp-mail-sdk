...
The Lua SDK for the TempMail API. Provides an entity-oriented interface using Lua conventions.
...
```lua
local result, err = client:Email(nil):list(nil, nil)
...
local result, err = client:TempMail(nil):load(
  { id = "test01" }, nil
)
