...
local TempMailSDK = {}
TempMailSDK.__index = TempMailSDK
...
TempMailSDK._make_feature = _make_feature


function TempMailSDK.new(options)
  local self = setmetatable({}, TempMailSDK)
  self.mode = "live"
  self.features = {}
  self.options = nil
...
function TempMailSDK:options_map()
...
function TempMailSDK:get_utility()
  return Utility.copy(self._utility)
end


function TempMailSDK:get_root_ctx()
  return self._rootctx
end


function TempMailSDK:prepare(fetchargs)
...
function TempMailSDK:direct(fetchargs)
...
-- by passing it.
function TempMailSDK:_raw_request(fetchargs)
  local utility = self._utility
...
function TempMailSDK:graphql(query, variables, ctrl)
  ...
  if type(errors) == "table" and 0 < #errors then
    ...
    res.ok = false
    res.err = "TempMailSDK: graphql: " .. msg
    res.graphql = errors
  end
...
-- Entity access is capitalised (PascalCase) for parity with the other SDKs.
function TempMailSDK:Mailbox(data)
...
function TempMailSDK.test(testopts, sdkopts)
  sdkopts = sdkopts or {}
  ...
  local sdk = TempMailSDK.new(sdkopts)
  sdk.mode = "test"
