...
class PrimaryUtilityTest < Minitest::Test
  ...
  def setup
    @runner = TempMailOmni.make_runner(TEST_JSON_FILE, TempMailSDK.test(nil, nil))
  ...
  def test_make_point_basic
    ...
    subject = lambda do |ctx|
      ...
      err.nil? ? point : TempMailOmni.errify(err)
    end
