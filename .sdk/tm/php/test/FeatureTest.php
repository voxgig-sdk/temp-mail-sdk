...
class FtCtrl extends TempMailControl
{
...
class FtClient
{
    ...
    private TempMailUtility $utility;

    public function __construct(TempMailUtility $utility, array $options)
    {
    ...
    public function get_utility(): TempMailUtility
    {
        return $this->utility;
    }
}
...
class FtRecorder
{
    ...
    {
        return function (TempMailContext $_ctx, string $url, array $fetchdef): array {
...
class FtHarness
{
    ...
    public TempMailContext $rootctx;
    ...
    public function __construct(
        ...
        $utility->fetcher = $server ?? function (TempMailContext $_ctx, string $_url, array $fetchdef): array {
        ...
        foreach ($features as $fspec) {
            ...
            $f = TempMailConfig::make_feature($name);
    ...
    private static function buildUrl(TempMailSpec $spec): string
    {
        $keys = [];
    ...
    public function op(array $o = []): array
    {
        ...
        if (($ctx->out['point'] ?? null) instanceof TempMailError) {
        ...
        if (is_array($fetched)) {
            $ctx->response = new TempMailResponse($fetched);
        }
    ...
    private function fail(TempMailContext $ctx, mixed $err): array
    {
...
class FeatureTest extends TestCase
{
    ...
    public function test_retry_retries_a_transport_error_until_exhausted(): void
    {
        ...
        $clock = new FtClock();
        $n = 0;
        $server = function (TempMailContext $ctx, string $_url, array $_fd) use (&$n): array {
            $n++;
