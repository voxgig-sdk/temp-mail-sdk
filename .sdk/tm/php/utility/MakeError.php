...
class TempMailMakeError
{
    public static function call(?TempMailContext $ctx, mixed $err): mixed
    {
        if ($ctx === null) {
            require_once __DIR__ . '/../core/Context.php';
            $ctx = new TempMailContext([], null);
        }
        $op = $ctx->op ?? new TempMailOperation([]);
        $opname = $op->name;
        if ($opname === '' || $opname === '_') {
            $opname = 'unknown operation';
        }

        $result = $ctx->result ?? new TempMailResult([]);
        $result->ok = false;
        ...
        $errmsg = ($err instanceof TempMailError) ? $err->msg : (string)$err;
        ...
        $sdk_err = new TempMailError('', $msg, $ctx);
