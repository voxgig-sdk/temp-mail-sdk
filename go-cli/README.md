...
boru-driven command-line client **and** interactive REPL for the TempMail
...
```sh
# 1. Build a native binary (-> dist/<os>-<arch>/temp-mail-cli)
make build
...
./temp-mail-cli list email
./temp-mail-cli list mailbox

# 5. Override the API base URL for a single call
TEMP_MAIL_BASE=https://api.example.com ./temp-mail-cli list email

# 6. No arguments -> interactive REPL
./temp-mail-cli
temp-mail> list email
temp-mail> /quit
```
...
1. **Build the binary.** From this `go-cli/` directory:
   ...
   ```sh
   make build          # -> dist/<os>-<arch>/temp-mail-cli
   ```
...
Run with no arguments to open a REPL (prompt `temp-mail>`). Each line is
...
| Target | Result |
|--------|--------|
| `make build` | Native binary at `dist/<os>-<arch>/temp-mail-cli`. |
...
(`./temp-mail-cli <expr>`) and interactively (the REPL), and expressions compose the
