// Command TempMail-cli is a CLI + REPL for the TempMail SDK,
...
const prompt = "temp-mail"

// entitiesHelp is the space-separated entity list shown by /help.
const entitiesHelp = "email mailbox"
...
func usage(out io.Writer) {
	...
	fmt.Fprintln(out, "Env:      TEMP_MAIL_APIKEY (api key), TEMP_MAIL_BASE (base url override)")
...
func run(args []string, in io.Reader, out, errOut io.Writer) int {
	...
	// Configure from the environment: TEMP_MAIL_APIKEY carries the API key and
	// TEMP_MAIL_BASE optionally overrides the API base URL (e.g. production).
	// Both injectable by a secrets vault. Unset -> nil config defaults.
	var opts map[string]any
	if apikey := os.Getenv("TEMP_MAIL_APIKEY"); apikey != "" {
		opts = map[string]any{"apikey": apikey}
	}
	if base := os.Getenv("TEMP_MAIL_BASE"); base != "" {
		if opts == nil {
			opts = map[string]any{}
		}
		opts["base"] = base
	}
	client := sdk.NewTempMailSDK(opts)
