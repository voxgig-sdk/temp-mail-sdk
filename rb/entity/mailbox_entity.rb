...
class MailboxEntity
  ...
  def make
    opts = @_entopts.dup
    MailboxEntity.new(@_client, opts)
  end

  def data_set(args)
    if args
      @_data = TempMailHelpers.to_map(VoxgigStruct.clone(args)) || {}
      @_utility.feature_hook.call(@_entctx, "SetData")
    end
  end

  # @return [Mailbox, Hash] the current Mailbox data
  def data_get
  ...
  def match_set(args)
    if args
      @_match = TempMailHelpers.to_map(VoxgigStruct.clone(args)) || {}
      @_utility.feature_hook.call(@_entctx, "SetMatch")
    end
  end

  # @return [Hash] the current match filter (any subset of Mailbox fields)
  def match_get
  ...
  #
  # @param reqdata [MailboxCreateData, Hash, nil] body data
  # @param ctrl [Object, nil] optional per-call control
  # @return [Mailbox, Hash] the created Mailbox; raises TempMailError on failure
    ...
    _run_op(ctx) do
      if ctx.result
        if ctx.result.resdata
          @_data = TempMailHelpers.to_map(VoxgigStruct.clone(ctx.result.resdata)) || {}
        end
      end
    end
  end
