...
module TempMailUtilities
  MakeRequest = ->(ctx) {
    ...
    if fetch_err
      ...
      response = TempMailResponse.new({ "err" => ctx.make_error("request_no_response", "response: undefined") })
    elsif fetched.is_a?(Hash)
      response = TempMailResponse.new(fetched)
    else
