...
func (sdk *TempMailSDK) OptionsMap() map[string]any {
...
func (sdk *TempMailSDK) GetUtility() *Utility {
	return CopyUtility(sdk.utility)
}

func (sdk *TempMailSDK) GetRootCtx() *Context {
	return sdk.rootctx
}

func (sdk *TempMailSDK) Prepare(fetchargs map[string]any) (map[string]any, error) {
	utility := sdk.utility
...
func (sdk *TempMailSDK) Direct(fetchargs map[string]any) (map[string]any, error) {
...
func (sdk *TempMailSDK) opAllowed(op string) bool {
	allowOp, _ := vs.GetPath([]any{"allow", "op"}, sdk.options).(string)
	return strings.Contains(allowOp, op)
}

func (sdk *TempMailSDK) opDenied(op string) map[string]any {
	allowOp, _ := vs.GetPath([]any{"allow", "op"}, sdk.options).(string)
	return map[string]any{
		"ok": false,
		"err": fmt.Errorf("TempMailSDK: %s: operation not allowed by"+
...
// gate by passing it.
func (sdk *TempMailSDK) rawRequest(fetchargs map[string]any) (map[string]any, error) {
	utility := sdk.utility
...
func (sdk *TempMailSDK) Graphql(
	...
	if 0 < len(errors) {
		...
		res["ok"] = false
		res["err"] = fmt.Errorf("TempMailSDK: graphql: %s", msg)
		res["graphql"] = errors
	}
