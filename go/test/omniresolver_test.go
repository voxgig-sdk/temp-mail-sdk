...
import (
	...
	omni "github.com/voxgig-sdk/temp-mail-sdk/go/test/omni"
...
func omniProvider(client any) *omni.Provider {
	provider := &omni.Provider{
		...
		Errify: func(err any) any {
			if sdkerr, is := err.(*sdk.TempMailError); is {
				out := map[string]any{
					"name":    "TempMailError",
...
func omniCtx(arg any, client *sdk.TempMailSDK, utility *sdk.Utility) *sdk.Context {
	...
	if provider, is := ctxmap["client"].(*omni.Provider); is {
		if live, is := defproviderclient(provider).(*sdk.TempMailSDK); is {
			client = live
