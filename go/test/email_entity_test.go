...
func TestEmailEntity(t *testing.T) {
	...
	t.Run("basic", func(t *testing.T) {
		...
		if setup.syntheticOnly {
			t.Skip("live entity test uses synthetic IDs from fixture — set TEMPMAIL_TEST_EMAIL_ENTID JSON to run live")
			return
		}
		client := setup.client
...
func emailBasicSetup(extra map[string]any) *entityTestSetup {
	...
	entidEnvRaw := os.Getenv("TEMPMAIL_TEST_EMAIL_ENTID")
	...
	idmapResolved := core.ToMapAny(env["TEMPMAIL_TEST_EMAIL_ENTID"])
