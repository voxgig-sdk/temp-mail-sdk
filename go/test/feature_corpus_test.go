...
//
// NewTempMailSDK, not TestSDK: the `test` feature is transport: 'base' and
// REPLACES the transport, so a client in test mode would shadow the script.
func fcClient(kase map[string]any) *sdk.TempMailSDK {
	res, _ := kase["res"].([]any)
	opts := map[string]any{
	...
	return sdk.NewTempMailSDK(opts)
}
...
func fcCandidates(client *sdk.TempMailSDK) []fcOp {
	out := []fcOp{}
...
func fcInvoke(client *sdk.TempMailSDK, op fcOp, ctrl map[string]any) error {
	cv := reflect.ValueOf(client)
	ct := cv.Type()
	mapType := reflect.TypeOf(map[string]any{})
...
// new go here.
func fcRecord(client *sdk.TempMailSDK, name string) any {
