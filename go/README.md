...
The Golang SDK for the TempMail API — an entity-oriented client using standard Go conventions. No generics required; data flows as `map[string]any`.
...
## Install
```bash
go get github.com/voxgig-sdk/temp-mail-sdk/go@latest
```

The Go module proxy resolves the version from the `go/vX.Y.Z` GitHub
release tag — see [Releases](https://github.com/voxgig-sdk/temp-mail-sdk/releases) for the available versions.
...
```bash
go mod edit -replace github.com/voxgig-sdk/temp-mail-sdk/go=../temp-mail-sdk/go
```
...
func main() {
    ...
    // List email records — the value is the array of records itself.
    emails, err := client.Email(nil).List(nil, nil)
    if err != nil {
        panic(err)
    }
    for _, item := range emails.([]any) {
        fmt.Println(item)
    }
}
```
...
| `Mailbox` | `(data map[string]any) TempMailEntity` | Create a Mailbox entity instance. |
...
slice):
    ...
    email, err := client.Email(nil).List(map[string]any{/* fields */}, nil)
    if err != nil { /* handle */ }
...
```
github.com/voxgig-sdk/temp-mail-sdk/go/
├── temp-mail.go        # Root package — type aliases and constructors
...
The root package (`github.com/voxgig-sdk/temp-mail-sdk/go`) re-exports everything needed
