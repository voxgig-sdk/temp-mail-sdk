    ...
    def test_should_run_basic_flow(self):
        ...
        if setup.get("synthetic_only"):
            pytest.skip("live entity test uses synthetic IDs from fixture — "
                        "set TEMP_MAIL_TEST_MAILBOX_ENTID JSON to run live")
        client = setup["client"]
        ...
        mailbox_ref01_data = helpers.to_map(runner.entity_data(mailbox_ref01_ent.create(mailbox_ref01_data, None)))
        assert mailbox_ref01_data is not None
        assert mailbox_ref01_data["id"] is not None
...
def _mailbox_basic_setup(extra):
    ...
    idmap_resolved = helpers.to_map(
        env.get("TEMP_MAIL_TEST_MAILBOX_ENTID"))
    if idmap_resolved is None:
        idmap_resolved = helpers.to_map(idmap)

    if env.get("TEMP_MAIL_TEST_LIVE") == "TRUE":
        merged_opts = vs.merge([
            {
                "apikey": env.get("TEMP_MAIL_APIKEY"),
            },
            extra or {},
        ])
        client = TempMailSDK(helpers.to_map(merged_opts))

    _live = env.get("TEMP_MAIL_TEST_LIVE") == "TRUE"
    return {
        "client": client,
        "data": entity_data,
        "idmap": idmap_resolved,
        "env": env,
        "explain": env.get("TEMP_MAIL_TEST_EXPLAIN") == "TRUE",
        "live": _live,
