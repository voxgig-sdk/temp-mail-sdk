...
class TempMailTestRunner:
    ...
    def load_env_local():
        ...
        for line in content.splitlines():
            ...
            TempMailTestRunner._env[key] = val

    @staticmethod
    def getenv(key):
        val = TempMailTestRunner._env.get(key)
        if val is not None:
            return val
    ...
    @staticmethod
    def env_override(m):
        live = TempMailTestRunner.getenv("TEMP_MAIL_TEST_LIVE")
        override = TempMailTestRunner.getenv("TEMP_MAIL_TEST_OVERRIDE")

        if live == "TRUE" or override == "TRUE":
            for key in list(m.keys()):
                envval = TempMailTestRunner.getenv(key)
        ...
        explain = TempMailTestRunner.getenv("TEMP_MAIL_TEST_EXPLAIN")
        if explain is not None and explain != "":
            m["TEMP_MAIL_TEST_EXPLAIN"] = explain
    ...
    def load_test_control():
        ...
        """
        if TempMailTestRunner._test_control is not None:
            return TempMailTestRunner._test_control
        ctrl_path = os.path.join(os.path.dirname(__file__), "sdk-test-control.json")
        try:
            with open(ctrl_path, "r") as f:
                TempMailTestRunner._test_control = json.load(f)
        except (FileNotFoundError, IOError, ValueError):
            TempMailTestRunner._test_control = {
                "version": 1,
                "test": {"skip": {
    ...
    def is_control_skipped(kind, name, mode):
        ...
        ctrl = TempMailTestRunner.load_test_control()
...
def load_env_local():
    TempMailTestRunner.load_env_local()


def env_override(m):
    return TempMailTestRunner.env_override(m)


def entity_data(v):
    return TempMailTestRunner.entity_data(v)


def entity_list_to_data(lst):
    return TempMailTestRunner.entity_list_to_data(lst)


def is_control_skipped(kind, name, mode):
    return TempMailTestRunner.is_control_skipped(kind, name, mode)


def load_test_control():
    return TempMailTestRunner.load_test_control()


def live_client_options():
    return TempMailTestRunner.live_client_options()


def live_delay_ms():
    return TempMailTestRunner.live_delay_ms()
